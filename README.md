# 🚀 Git Push Time CLI (`git-push-time`)
...
Trong thư mục dự án này, chạy câu lệnh sau để cài đặt lệnh `git-push-time` vào máy:
```bash
npm install -g .
```
Sau khi cài đặt, bạn có thể gọi lệnh `git-push-time` hoặc `git-custom-push` ở **bất kỳ thư mục dự án nào** trên máy tính!

### Cách 2: Chạy trực tiếp qua `npx` hoặc Node
```bash
npx .
# Hoặc
node bin/index.js
